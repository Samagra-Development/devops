...
source <(grep "^WEBHOOK_SECRET=" .env)
if [ -z "$WEBHOOK_SECRET" ]; then
    echo "ERROR: WEBHOOK_SECRET is not defined in .env"
    exit 1
fi
...
tmux new-session -c "$PROJECT_ROOT" -d -s webhook-session "export WEBHOOK_SECRET=$WEBHOOK_SECRET ; webhook -hooks $PROJECT_ROOT/scripts/webhook/hooks.json -verbose -template 2>&1 | tee -a \"$PROJECT_ROOT/deploy.log\"; bash"
