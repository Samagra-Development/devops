...
REMOVE_ORPHANS_FLAG := $(if $(and $(filter-out 1,$(DISABLE_REMOVE_ORPHANS)),$(services)),--remove-orphans,)
REMOVE_ANSI_FLAG := $(if $(filter-out 1,$(DISABLE_ANSI)),--ansi never,)
...
deploy: $(if $(filter 1,$(ENABLE_GIT_PULL)),git-pull,) $(if $(filter-out 1,$(DISABLE_PULL)),pull build,) reload-caddy
